...
app.get("/health", (req, res) => {
  res.json({
    status: "ok",
    tvs: Object.keys(activeTVs).length,
    pcs: Object.keys(activePCs).length,
  });
});
...
wss.on("connection", (socket, req) => {
  ...
  socket.on("message", (message) => {
    ...
    if (data.type === "register-tv") {
      ...
      if (
        existingCode &&
        Date.now() - activeTVs[existingCode].timestamp < CODE_EXPIRY_TIME
      ) {
        logger.info(
          `Returning existing code: ${existingCode} for ${socket.id}`
        );
        clientCode = existingCode;
        notifyClient(socket, "tv-registered", "TV registered", {
          code: existingCode,
        });
      } else {
        // Generate a new code
    ...
    // Handle PC Verification
      ...
      if (
        activeTVs[code] &&
        activeTVs[code].socket.readyState === WebSocket.OPEN
      ) {
        ...
        notifyClient(
          socket,
          "code-verified",
          "Code verified. Ready to start casting."
        );
        notifyClient(
          activeTVs[code].socket,
          "pc-paired",
    ...
    // Handle WebRTC Offer (PC → TV)
      ...
      logger.info(
        `Relaying WebRTC offer from PC ${socket.id} to TV ${clientCode}`
      );
      notifyClient(
        activeTVs[clientCode].socket,
        "offer",
    ...
    // Handle WebRTC Answer (TV → PC)
      ...
      if (
        clientType !== "tv" ||
        !clientCode ||
        !activeTVs[clientCode]?.pcConnection ||
        !data.answer
      ) {
    ...
    // Handle ICE Candidates
      ...
      if (clientType === "pc" && clientCode && activeTVs[clientCode]) {
        logger.info(
          `Relaying ICE candidate from PC ${socket.id} to TV ${clientCode}`
        );
        notifyClient(
          activeTVs[clientCode].socket,
          "ice-candidate",
    ...
    // Handle Stop Screen Cast
      ...
      logger.info(
        `Relaying stop screen cast from PC ${socket.id} to TV ${clientCode}`
      );

      notifyClient(
        activeTVs[clientCode].socket,
        "stop-screen-cast",
        "PC initiated disconnect",
        {
          action: "disconnect",
      ...
      notifyClient(socket, "cast-stopped-ack", "Screen cast stopped", {
        timestamp: Date.now(),
      });
      logger.info(
        `Screen cast stopped for code ${clientCode}, but pairing remains active`
      );
    }
    ...
    // Handle explicit casting session end (complete disconnect)
      ...
      notifyClient(
        activeTVs[clientCode].socket,
        "session-ended",
        "Casting session ended. Return to pairing screen."
      );
    ...
    // Handle TV-initiated streaming end request
      ...
      notifyClient(
        socket,
        "end-streaming-request-ack",
        "End streaming request sent",
        { timestamp: Date.now() }
      );
    }
    ...
    // Handle TV ready signal
      ...
      if (clientType !== "tv" || !clientCode) {
        notifyClient(
          socket,
          "error",
          "Invalid tv-ready-for-new-connection request"
        );
        return;
      }
